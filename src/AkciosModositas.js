...
import './AkciosModositas.css';

export default function AkciosModositas() {
    const [property, setProperty ] = useState({
      ...
      useEffect(() => {
        ...
        const fetchProperty = async () => {
          try {
            const response = await axios.get(`http://192.168.10.113:5149/api/Akciók/${id}`);
      ...
      const handleUpdate = async () => {
        try {
          ...
          const response = await axios.put(`http://192.168.10.113:5149/api/Akciók/${id}`, updatedProperty);
          console.log(response);
          alert("Sikeres frissítés!");
          navigate('/Akcioshazak');
        } catch (error) {
